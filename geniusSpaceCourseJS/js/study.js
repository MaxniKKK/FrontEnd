...
//
// В цьому прикладі наслідки не такі жахливі. Однак в інших випадках
// ми можемо встановлювати об'єктні значення і тоді прототип може
// дійсно змінитись. В результаті, виконання коду піде неправильним
// та неочікуваним шляхом.
// Найгірше в цьому - зазвичай розробники не задумуються над тим,
// що така ситуація взагалі можлива. Це робить подібні помилки
// важкими для виявлення і перетворюються їх у вразливості,
// особливо коли JavaScript використовується на стороні серверу.
//
// Неочікувані речі можуть траплятися при встановлені значення
// для властивості toString, яка за замовчуванням є функцією,
// та для інших вбудованих методів.
//
// Як ми можемо уникнути цієї проблеми?
// В першу чергу, для зберігання ми можемо просто використовувати
// Map замість простих об'єктів, тоді все працює правильно.
//
// Проте Object може також послужити нам, оскільки творці мови
// задумувались над цією проблемою вже дуже давно.
//
// __proto__ не є властивістю об'єкту, але є аксесором Object.prototype.
// Таким чином, якщо obj.__proto__ зчитується або встановлюється,
// відповідний гетер/сетер викликається та отримує/встановлює [[Prototype]]
// Як було сказано на початку цього розділу: __proto__ це спосіб доступу
// до [[Prototype]], але не є самим [[Prototype]]
//
// Тепер, коли ми хочемо використати об'єкт як асоціативний масив
// та уникнути таких проблем, ми можемо зробити це за допомогою невеликої
// хитрості:
/*
let obj = Object.create(null);
let key = prompt('enter a key', '__proto__');
obj[key] = 'some value';
alert(obj[key]); // some value
*/
// Object.create(null) створює пустий об'єкт без прототипу([[Prototype]] дорівнює null)
// Таким чином, відсутній наслідувані гетер/сетер для __proto__.
...
// --------------------------------------------------------------
//                  Якість коду
// • Використовуйте зрозумілі імена змінних та функцій.
//    Код набагато легше читати, коли в його написанні використовують
//    зрозумілі, описові імена функцій і змінних.
// • Пишіть короткі функції, які вирішують одне завдання.
//    Функції легше підтримувати, вони стають набагато зрозумілішіми,
//    читальнішими, якщо вони спрямовані на вирішення лише одного завдання.
// • Пам'ятайте про принцип DRY.
//    Одна з перших ідей, яку намагаються донести до того, хто хоче
//    стати програмістом, звучить так: не повторюйся(Dont Repeat Yourself - DRY)
//    Якщо ви помічаєте у своїх проектах фрагменти, що повторюються - використовуйте
//    такі програмні конструкції, які дозволять скоротити повтори одного і того ж коду.
// • Використовуйте ідеї інкапсуляції та модульності.
//    Групуйте пов'язані змінні та функції для того, щоб зробити ваш код
//    зрозумілішим та покращити його з точки зору його повторного використання.
// • Документуйте код
//    Пишіть хорошу документацію для свого коду - тоді той, хто зіткнеться з ним
//    у майбутньому, зрозуміє, що і чому в цьому коді робиться.
// • Подумайте про використання правил Сенді Метц.
//    Сенді Метц чудово програмує на Ruby, робить цікаві доповіді та пише книги.
//    Вона сформулювала чотири правила написання чистого коду об'єктно-орієнтованими мовами.
//    Ось вони:
//      1. Класи не повинні бути довшими за 100 рядків коду.
//      2. Методи та функції не повинні бути довшими за 5 рядків коду.
//      3. Методам слід передавати трохи більше 4 параметрів
//      4. Контролери можуть ініціалізувати лише один об'єкт.
